...
def plot_curve(log_dicts, args):
    ...
    metrics = args.keys
    #metrics = ["decode_source.loss_seg",
    #           "decode_target.loss_seg",
    #           "feature_adaptation.loss_consistency",
    #           "feature_adaptation.loss_feat",
    #           "feature_adaptation.loss_discr",
    #           "loss"]
    #print(metrics)
    #exit()
    ...
    for i, log_dict in enumerate(log_dicts):
        ...
        for j, metric in enumerate(metrics):
            ...
            if metric in ['mIoU', 'mAcc', 'aAcc']:
                ...
                plt.xlabel('iter')
                plt.plot(plot_iters, plot_values, label=label, linewidth=1.0)
        plt.legend()
...
def main():
    ...
    args.keys = ["decode_source.loss_seg",
                 "decode_target.loss_seg",
                 "feature_adaptation.loss_consistency",
                 "feature_adaptation.loss_feat",
                 "feature_adaptation.loss_discr",
                 "loss",
                 'feature_adaptation.discr_acc']
